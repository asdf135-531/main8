        ...
        for event in range(self.N_events):
            ...
            while E > 0:
                ...
                if interaction_type == 'ph':
                  if E <= self.source_energy:  # защита (на всякий случай)
                      channel = int((E - self.E_min) / self.Cch)
                  ...
                  break
                    ...
                    if dE > 0 and dE <= self.source_energy:
                      channel = int((dE - self.E_min) / self.Cch)
                      if 0 <= channel < self.num_channels:
                          self.spectrum[channel] += 1
                    E = E - dE
